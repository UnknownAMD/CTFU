...
namespace EventPreprocessor
{
    public class FTEventViewer : IFusionTool
    {
        ...
        public void ProcessCondition(Condition condition)
        {
            ...
            if (!gotHandlers)
            {
                if ($"Object not implemented: {(Constants.ObjectType)condition.ObjectType}({condition.ObjectType})" == "Object not implemented: SubApplication(9)") WriteLine("Sub Application Condition");
                if ($"Object not implemented: {(Constants.ObjectType)condition.ObjectType}({condition.ObjectType})" == "Object not implemented: Extension(32)") WriteLine("Active System Box Interaction");
                if ($"Object not implemented: {(Constants.ObjectType)condition.ObjectType}({condition.ObjectType})" == "Object not implemented: Timer(-4)") WriteLine("Timer Equals");
                return;
            }
            ...
            if (currentHandlers != null)
            {
                ...
                else
                {
                    try
                    {
                        if (ConditionNames.ConditionSystemDict[condition.ObjectType][condition.Num] == "MenuSelected")
                        {
                            object Parametr = null;
                            foreach (var param in condition.Items)
                            {
                                Parametr = param.Value;
                            }
                            WriteLine("Menu Option " + Parametr + " Selected");

                        }
                        else
                        {
                            WriteLine(
                                $"{ConditionNames.ConditionSystemDict[condition.ObjectType][condition.Num]}");
                        }
